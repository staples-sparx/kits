...
(deftest test->str
  ...
  (testing "with specified timezone"
    (is (= "2012-01-30 05:00:00"
           (->str (->timestamp "01 30, 2012" "MM dd, yyyy" "America/New_York"))))))
  ...
  (testing "Calendar units are (milli second minute hour day month week year)"
    ...
    (testing "hour ranges"
      (is (= [[(->timestamp "2000-01-01 12:00:00")
             ...
             (timestamp-ranges "2000-01-01 12:00" "2000-01-01 14:00" "minute-60")
             )))
    ...
    ;; (testing ":no-range-overflow"
    ;;   '("2014-07-29 21:45:29" "2014-07-29 22:30:39")
    ;;   (is
    ;;    (= [[(->timestamp "2000-01-01 14:00:00")
    ;;         (+ (->timestamp "2000-01-01 14:44:59") 999)]]
    ;;       (timestamp-ranges "2000-01-01 12:00" "2000-01-01 12:45" "minute-60")
    ;;       (timestamp-ranges "2000-01-01 12:00" "2000-01-01 12:45" "hour"))))
    ))
